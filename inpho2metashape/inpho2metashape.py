...
SENSOR_SIZE = 212.005 # [mm]
CC_API_path = Path(r"C:\Users\Luscias\Desktop\3DOM\Github_3DOM\COLMAP_scripts\AlignCC_for_windows")
imgs_dir = Path(r"C:\Users\Luscias\Desktop\ElisaPolonia\inpho2metashape_regular_scanner\imgs\1986_05_05-06_1-3000_regular_scanner")
prj_file = Path(r"C:\Users\Luscias\Desktop\ElisaPolonia\inpho2metashape_regular_scanner\inpho_prj_AT_regular.prj")
...
for img_dict in database:
    with open("./temp_inpho.txt", 'w') as inpho, open("./temp_meta.txt", 'w') as meta:
        ...
        #dist_pixel = np.sqrt((float(fid_1[1])-float(fid_0[1]))**2+(float(fid_1[2])-float(fid_0[2]))**2)
        #print(dist_pixel)
